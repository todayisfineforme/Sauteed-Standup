...
function Tickets(props){
    ...
    for (var i= 0; i<upcomingDates.length; i++){
        ...
        if (current < next){
            nextShow = months[nextMonth] + " " + days[nextDay];
            break;
        };
    };
    ...
    return(
        <Container className={props.containerSpace} fluid>
            ...
            <Row className={props.rowSpace}>
                <Button variant="outline-light" size="lg" href="https://www.simpletix.com/e/sauteed-standup-tickets-77571" target="_blank"><h1>Sunday <span>{nextShow}</span></h1></Button>
            </Row>
        </Container>
    );
}
